  ...
  await pauseConsumer.run({
    eachMessage: async ({ topic, partition, message }) => {
      ...
      switch (paramsMessage.code) {
        case 0:
          request.post({url: uriWebsockerService + '/play', form: {clientIds: clients}});
          console.log('request play');
          break;

        case 1:
          request.post({url: uriWebsockerService + '/pause', form: {clientIds: clients}});
          console.log('request pause');
          break;

        case 2:
          request.post({url: uriWebsockerService + '/settimecode', form: {clientIds: clients, timecode: paramsMessage.timecode}});  
